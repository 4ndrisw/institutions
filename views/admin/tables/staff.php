...
//$where = ['client_id' => $institution->userid];
$where = [];
//$where = ['AND client_id=' . $this->ci->db->escape_str($client_id)];
//$where = ['AND client_type='. '"institution"'];

array_push($where, 'AND client_id=' . $this->ci->db->escape_str($client_id));
array_push($where, 'AND client_type='. '"institution"');

//$where = hooks()->apply_filters('institutions_staff_table_sql_where', []);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'last_login') {
            ...
            if($has_pengguna_edit){
                $_data = '<a href="' . admin_url('institutions/staff/profile/' . $aRow['staffid']) . '">' . staff_profile_image($aRow['staffid'], [
                    'staff-profile-image-small',
                    ]) . '</a>';
                $_data .= ' <a href="' . admin_url('institutions/staff/member/' . $aRow['staffid']) . '">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>';

                $_data .= '<div class="row-options">';
                $_data .= '<a href="' . admin_url('institutions/staff/member/' . $aRow['staffid']) . '">' . _l('view') . '</a>';
    ...
    $row = hooks()->apply_filters('institutions_staff_table_row', $row, $aRow);
